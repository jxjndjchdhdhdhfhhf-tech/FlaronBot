...
def add_staff_points(user_id, amount=5, check_cooldown=False):
    ...
    if check_cooldown:
        ...
        # التحقق من مرور 30 دقيقة (1800 ثانية)
...
class RatingView(ui.View):
    ...
    async def rate_1(self, interaction, button): await self.send_thanks(interaction, 1)
    @ui.button(label="⭐⭐", style=ButtonStyle.secondary, custom_id="rate_2")
    async def rate_2(self, interaction, button): await self.send_thanks(interaction, 2)
    @ui.button(label="⭐⭐⭐", style=ButtonStyle.secondary, custom_id="rate_3")
    async def rate_3(self, interaction, button): await self.send_thanks(interaction, 3)
    @ui.button(label="⭐⭐⭐⭐", style=ButtonStyle.secondary, custom_id="rate_4")
    async def rate_4(self, interaction, button): await self.send_thanks(interaction, 4)
    @ui.button(label="⭐⭐⭐⭐⭐", style=ButtonStyle.secondary, custom_id="rate_5")
    async def rate_5(self, interaction, button): await self.send_thanks(interaction, 5)
...
class TicketActionsView(ui.View):
    ...
    async def close(self, interaction: discord.Interaction, button: ui.Button):
        # إضافة النقاط مع شرط الـ 30 دقيقة
...
# --- كلاس فتح التيكيت (الرسالة الترحيبية لم تتغير) ---
class TicketView(ui.View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: ui.Button):
        ...
        welcome_embed = discord.Embed(title=f"Welcome to {channel_name}", description="The support team will be with you shortly.", color=discord.Color.green())
        await channel.send(embed=welcome_embed, view=TicketActionsView())
        
        # الرسالة الترحيبية الكاملة (بقي كل شيء كما هو)
...
async def setup_ticket(ctx):
    embed = discord.Embed(title="⚔️ Flaron Mc | نظام التذاكر", description="يرجى الضغط على الزر أدناه لفتح تذكرة جديدة.", color=discord.Color.dark_gray())
